export type PixelPosition = { x1: number, y1: number, x2: number, y2: number };

export function dynamicPixelMatch(img1: Buffer, img2: Buffer, output: Buffer, width: number, height: number, options: any, excluded: PixelPosition[] = []) {
    if (!options) options = {};
    ...
    const calculatePixelsFromPercents = ({ width, height }: { width: number, height: number }) => ({ x1, y1, x2, y2 }: { x1: number, y1: number, x2: number, y2: number }) => ({
...
function antialiased(img: Buffer, x1: number, y1: number, width: number, height: number, img2?: Buffer): boolean {
...
function colorDelta(img1: Buffer, img2: Buffer, k: number, m: number, yOnly: boolean = false) {
    let a1 = img1[k + 3] / 255,
        a2 = img2[m + 3] / 255,
...
function grayPixel(img: Buffer, i: number) {
    let a = img[i + 3] / 255,
        r = blend(img[i], a),
        g = blend(img[i + 1], a),
        b = blend(img[i + 2], a);
